...
- **双链 `[[...]]`**：markdown-it 自定义插件，解析为可点击链接。显示别名 `[[目标|别名]]` 与 `[[目标#标题]]` 的解析**已实现**；跳转按 vault 内文件名（basename）匹配，**frontmatter 声明的别名尚未支持**——`#5` 曾为此开票，实测全库声明了别名的笔记只有 1 篇、被 `[[...]]` 指向 0 次，已按 `wontfix` 关闭；重开的触发条件是「有笔记声明的别名被实际引用」。（本节原先写「别名留后续」，措辞含混到分不清指哪一种，已按实现校正。）
...
- **引用解析仍未覆盖的**：只写 basename、文件在别处的图片引用（如 `![](x.png)` 而文件在 `assets/`）——需要按 vault 内文件名反查，属另一套索引，见 #8。
