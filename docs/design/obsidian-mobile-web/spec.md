...
>
> **时效**：2026-08-29 定稿，2026-09-13 校正。校正内容——部署平台由 EdgeOne Pages 改为 Deno Deploy（MVP 已上线）、补齐实现期新增的访问密码鉴权、字符级 diff 由「计划引入 diff-match-patch」改为「已自实现」。同目录的 `issues/` 与 `research/` 是**历史快照**，保留原貌不动，其中平台选型的结论已被本文件取代。
...
- **双链 `[[...]]`**：markdown-it 自定义插件，解析为可点击链接；跳转按 vault 内文件名匹配（先做简单文件名匹配，别名留后续）。
- **YAML frontmatter**：解析并在阅读态顶部展示（不参与编辑冲突的 diff）。
- **基础 GFM**：表格、代码块、列表、引用（markdown-it 默认能力）。
- **图片**：`![](path)` 渲染时 src 指向 `/api/raw?path=`（经后端代理，token 不进 URL）。
...
- 冲突长文本滚动 / 对照的真机体验（票 04 遗留，待真机验证）。
- 双链别名解析、图片多级目录等实现细节。
- 后端鉴权层的 `getToken()` 抽象边界（为 v2 OAuth 预留，不必现在实现）。
