...
- **引用解析仍未覆盖的**：只写 basename 的图片引用（Obsidian 的「能短则短」格式写出来就是 `![](x.png)`，文件实际落在 `assets/` 之类别的目录）——本端按相对路径找必然失败，需要按 vault 内文件名反查。**原句「属另一套索引」已过期**：`#7` 落地后索引已含非 `.md` 文件的 basename → 路径，反查与双链的 basename 匹配是**同一份数据**——不需要第二套索引，也不新增请求。实测（归档 `c44cab8`，829 篇）：117 条指向库内的图片引用里 **107 条解析不到**，其中 **100 条**按文件名唯一命中（落点全在根 `assets/`，命名全为 Obsidian 粘贴图自动生成的形态）、7 条是未填的占位与代码内示例（**不是真引用**）；涉及 21 篇笔记。判定与失败态沿用双链那一套（相对解析失败且索引未就绪 → 中性占位，就绪后补一次渲染；「拿不到」与「确实没有」分开），**待 #8 实现**。
